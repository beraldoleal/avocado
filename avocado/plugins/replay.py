...
class Replay(CLI):
    ...
    def run(self, config):
        job_id = future_settings.get('run.replay', 'job_id')
        if job_id is None:
            return

        err = None
        replay_ignore = future_settings.get('run.replay', 'ignore')
        test_status = future_settings.get('run.replay', 'test_status')
        ...
        # supplied.
        if future_settings.get('run.replay', 'resume'):
            if not test_status:
