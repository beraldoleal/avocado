...
class NRun(CLICmd):
    ...
    def run(self, config):
        resolutions = resolver.resolve(settings.get('nrun', 'references'))
        ...
        if not settings.get('nrun', 'disable_task_randomization'):
        ...
        try:
            ...
            listen = settings.get('nrun.status_server', 'listen')
